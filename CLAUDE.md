...
- Tasks are add + toggle only — no edit or delete yet. Only one open (uncompleted) task at a time; the add form is hidden until it's finished.
- **"Today" is a reactive `currentDate` ref** (`App.vue`), not a plain function call — this is what lets `todaysTasks`/`pastDays`/`currentStreak` actually refresh when the calendar day changes. It's initialized once from `stats.dateStr()` and re-synced only on `visibilitychange`/`focus` (no polling/interval) — cheap, and self-heals whenever the user actually looks at a tab left open past midnight.
